#!/bin/bash
# Cool-stage launcher for the AMD/ROCm (n-210) environment.
# Args:
#   $1 = config name (e.g. codecslime_cool_vq8k_n210)
#   $2 = run name / W&B id (e.g. cool-vq8k-n210-12500)
set -euo pipefail
...
if [ -f "$LOG_DIR/last.ckpt" ]; then
  ...
  echo "[auto_resume_cool_amd] resuming from $LOG_DIR/last.ckpt"
fi
